...
export class GitHubPRAnalysisService {
  ...
  private async getChangedFiles(
    ...
    try {
      ...
      const response = await this.githubApp.authenticatedRequest(
        installationIdNum,
        `/repos/${repositoryFullName}/pulls/${prNumber}/files`
      );
  ...
  private async getFileContent(
    ...
    try {
      ...
      const response = await this.githubApp.authenticatedRequest(
        installationIdNum,
        `/repos/${repositoryFullName}/contents/${filename}?ref=${sha}`
      );
  ...
  private async postPRComment(
    ...
    commentBody += `\n---\n*This analysis was generated by Flaky Test Detector. [Learn more](${process.env.FRONTEND_URL})*`;
